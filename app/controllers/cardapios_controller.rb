class CardapiosController < ApplicationController
  ...
  # POST /cardapios.json
  def create
    @cardapio = Cardapio.new(cardapio_params.merge(user: current_user))
  ...
  def destroy
    @cardapio.destroy
    respond_to do |format|
      format.html { redirect_to cardapios_url, notice: 'Cardapio was successfully destroyed.' }
  ...
  private
    ...
    def cardapio_params
      params.require(:cardapio).permit(:item)
    end
end
